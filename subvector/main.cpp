...
// увеличить емкость недовектора (можно использовать и для уменьшения - тогда, в рамках данной реализации, если top меньше новой capacity,
...
bool resize(subvector *qv, unsigned int new_capacity) {
    ...
    unsigned int skilka = qv->top;
    if (skilka > new_capacity) {
        skilka = new_capacity;
    }
        for (unsigned int i = 0; i < skilka; i++) {
            new_mas[i] = qv->mas[i];
        }
